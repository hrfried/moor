...
func TestOverflowDown(t *testing.T) {
	...
	assert.Equal(t, "hej", renderedToString(rendered[0]))
...
func TestShortenedInput(t *testing.T) {
	...
	assert.Equal(t, "first", renderedToString(rendered[0]))
...
func TestShortenedInputManyLines(t *testing.T) {
	...
	for i, row := range rendered {
		assert.Equal(t, renderedToString(row), expectedLines[i], "Line %d mismatch", i)
	}
	assert.Equal(t, pager.lineIndex().Index(), 90, "The last lines should now be visible")
	assert.Equal(t, "match 99", renderedToString(rendered[len(rendered)-1]))
}
